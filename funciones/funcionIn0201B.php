...
$csql = mysql_query("SELECT * FROM data05 WHERE id='$Data03'");
$crow = mysql_fetch_object($csql);


if($cData<=$crow->Existencia):

$DataC01 = $Data01." (".$Data01C.")";

$DataSQL00 = "insert into data09(Cantidad,id_user,Tipo,Motivo,IdProducto,FechaRegistro)
			         value('$Data01','$DataU01','$Data02','$Data00','$Data03','".date("Y-m-d H:i:s")."')";
...
$LogSql = "insert into data06(IdOperador, Registro, Módulo, IP, Estado)
		   value('$DataU01','Actualización de iventario','Inventario > Retirado: <b>$Data01AC $Data02</b> de <b>$DataROW01[NombreProducto]</b>','$ip','Éxitoso')";
mysql_query($LogSql);
...
if($cData>$crow->Existencia):
?>
<div id="ErrorExistencia" title="Aviso">
	<span style="color:red">Cantidad retirada supera a la existencia del producto.</span>
</div>
<script>
$("#ErrorExistencia").dialog({
	modal:true,
	close:function(){
		$(this).remove();
	}
